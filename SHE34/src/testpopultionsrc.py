...
def addProject(projectData):
    project = Project.objects.get_or_create(name=projectData['name'], description = projectData['description'],
                                            manager=User.objects.get(name=projectData['manager']))
    for evaluator in projectData['evaluator']:
        project.evaluators = User.objects.get(name=evaluator)
